...
// Bullet → Siv3D
inline s3d::Vec3 ToSiv3DVec3(const btVector3& v) {
    return s3d::Vec3(v.x(), v.y(), v.z());
}
inline s3d::Quaternion ToSiv3DQuaternion(const btQuaternion& q) {
    return s3d::Quaternion(q.x(), q.y(), q.z(), q.w());
}
